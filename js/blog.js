...
- ★★★ [검색으로 확인된 최신 팩트] 섹션에 없는 수치·날짜·인물·사건을 지어내는 것 ★★★
...
async function generateTistoryFromKeyword() {
  ...
  if (facts && facts.facts && facts.facts.length > 0) {
    factContext = `
★★★ 아래는 웹 검색으로 방금 확인한 최신 팩트입니다. ★★★
★★★ 글 작성 시 반드시 이 팩트들을 기반으로 하고, 여기 없는 수치·날짜·인물·확정 사실은 절대 지어내지 마세요. ★★★
★★★ 특히 개봉일, 출연 여부, 제작 상황 등 구체적 사실은 아래 팩트만 사용하세요. ★★★

[검색으로 확인된 최신 팩트]
${facts.facts.map((f, i) => `${i+1}. ${f}`).join('\n')}

${facts.caution && facts.caution.length > 0 ? `[주의 — 아래 내용은 불확실하므로 단정 짓지 말 것]\n${facts.caution.join('\n')}` : ''}

[핵심 요약]
${facts.summary || ''}

★★★ 위 팩트와 모순되는 내용을 쓰면 안 됩니다. ★★★
`;
  }
...
위 팩트를 기반으로 "${keyword}" 키워드의 SEO 최적화 블로그 글을 작성해주세요.
